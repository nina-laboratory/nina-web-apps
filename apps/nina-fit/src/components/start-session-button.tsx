  ...
  if (isSelectingDate) {
      return (
          <div className="w-full space-y-2 animate-in fade-in slide-in-from-top-2">
              <label htmlFor="retro-date" className="text-sm font-medium">Select Date</label>
              <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={handleStartRetroactive} disabled={!selectedDate}>Go</Button>    
              </div>
              <Button variant="ghost" size="sm" className="w-full text-xs" onClick={() => setIsSelectingDate(false)}>
                  Cancel
              </Button>
          </div>
      );
